...
"use strict"; if (!Imported || !Imported.OcRam_Core) alert('OcRam_Core.js ' +
    'is required!'); OcRam.addPlugin("Audio_EX", "2.11");

/*:
 * @plugindesc v2.11 This plugin provides dynamic sound positioning and adds 2 'generic' BGS channels which will also play in battle. PLUGIN NAME MUST BE OcRam_Audio_EX.js
 * @author OcRam
 *
 ...
 * 2021/04/08 v2.08 - "vanilla" SE/ME/BGS, BGS1 and BGS2 won't be stopped any
 *                    longer on player transfer (Credits: thepsyche & gRaViJa)
 * 2021/04/09 v2.09 - BGM volume option now works properly! (Credits: Foerster)
 * 2021/06/17 v2.10 - Fixed bug when faded BGM with 0% vol (Credits: Cyp999)
 * 2021/06/19 v2.11 - Allow same played BGS as current BGS parameter update
 *                    even if it's same BGS (Credits to Foerster)
 */
/*
 * ----------------------------------------------------------------------------
...
(function () {
    ...
    OcRam_Utils.clearBuffers = function (clear_all) {
    ...
    OcRam_Utils.cleanAllBuffers = function () {
        _processedBGS = []; OcRam_Utils.clearBuffers(true); _this._menuCalled = false;
    ...
    AudioManager.playBgm = function (pbgm, pos) {
        ...
        if (this.isCurrentBgm(bgm)) {
            ...
            this.stopBgm();
            if (bgm.name) {
                var target_vol = (AudioManager._bgmVolume * bgm.volume) / 100; var ov = bgm.volume;
                ...
                } bgm.volume = ov; this.fadeToBgm_OC(_defaultBgmFade, target_vol);
            }
